...
from pipeline.browser import get_browser_manager, retry_with_backoff
...
def _apply_linkedin(job, credentials, candidate, resume_path):
    """Apply via LinkedIn Easy Apply. Uses BrowserManager for screenshots/CAPTCHA detection."""
    ...
    # Try to start BrowserManager (graceful if Playwright missing)
    if not mgr.available:
        mgr.start()

    if not mgr.available:
        # Playwright not installed — return graceful skip
        return {"status": "skipped", "method": "linkedin", "error": "Playwright not available"}

    try:
        with mgr.page() as (page, ctx):
            if page is None:
                return {"status": "skipped", "method": "linkedin", "error": "Browser unavailable"}
            ...
            if challenge:
                mgr._save_screenshot(page, "linkedin_captcha")
                return {"status": "blocked", "method": "linkedin",
                        "error": f"Security challenge detected ({challenge}) — needs manual login"}

            if "checkpoint" in page.url or "challenge" in page.url:
                mgr._save_screenshot(page, "linkedin_blocked")
            ...
            if easy_apply.count() > 0:
                ...
                mgr._save_screenshot(page, "linkedin_partial")
