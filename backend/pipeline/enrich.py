...
def enrich_application(job, base_resume, base_cover, candidate, llm_config,
                       resume_mode="honest", style_index=0, role_template=None):
    """
    ...
    Args:
        ...
        role_template: dict from YAML template with ats_keywords, industry_context,
                       default_competencies, etc. Injected into prompts for field-aware output.
    ...
    template = role_template or {}

    # Step 1: Analyze the job description (with template context)
    analysis = _analyze_job(jd, llm_config, template)

    # Step 2: Generate tailored resume
    resume = _generate_resume(
        job, base_resume, candidate, llm_config,
        resume_mode, variant, analysis, template
    )
    if not resume:
        resume = base_resume
...
# Job analysis
# ---------------------------------------------------------------------------
def _analyze_job(jd, llm_config, template=None):
    """Extract key requirements from a job description, enriched by role template context."""
    template = template or {}

    # Inject template ATS keywords as hints for the analyzer
    template_keywords = ", ".join(template.get("ats_keywords", []))
    industry_ctx = template.get("industry_context", "")
    keyword_hint = ""
    if template_keywords:
        keyword_hint = f"\nField-specific keywords to watch for: {template_keywords}"
    if industry_ctx:
...
# Resume generation
# ---------------------------------------------------------------------------
def _generate_resume(job, base_resume, candidate, llm_config, mode, variant, analysis, template=None):
    """Generate a tailored resume based on the selected mode, with field-aware template context."""
    template = template or {}
    ...
    # Field-specific context from role template
    template_ats = ", ".join(template.get("ats_keywords", []))
    template_competencies = "\n".join(f"- {c}" for c in template.get("default_competencies", []))
    template_industry = template.get("industry_context", "")
    template_sections = ", ".join(template.get("default_resume_sections", []))

    field_context = ""
    if template_ats or template_industry:
        field_context = f"""
FIELD CONTEXT (use this to ensure the resume is appropriate for this industry):
- Industry: {template_industry}
- Field-specific ATS keywords to weave in naturally: {template_ats}
- Suggested competencies to reference where applicable:
{template_competencies}
- Suggested resume sections: {template_sections}
"""
...
You adapt your writing style, terminology, and emphasis based on the industry field.
...
RESUME FORMAT RULES — follow this markdown structure EXACTLY so every resume renders consistently:
- Line 1: `# {{Full Name}}`
- Line 2: one contact line: `{{City, Province}} | {{Phone}} | {{Email}}` (append ` | {{LinkedIn}}` if available)
- Then these sections, IN THIS ORDER, each titled with `## `:
  `## Professional Summary`, `## Core Competencies`, `## Professional Experience`, `## Education`, `## Certifications`
- Professional Summary: 2-3 sentences tailored to THIS role (plain text, no bullets).
- Core Competencies: 8-10 bullet lines (`- item`), weaving in the job's keywords and field-specific terms.
- Professional Experience: for EACH role, use exactly this shape:
    `### {{Job Title}} — {{Company}}`
    `*{{Location}} | {{Start}} – {{End}}*`
    then 3-4 bullet lines (`- ...`), each starting with a strong action verb and a metric where possible.
- Education: put EACH entry on its OWN line: `**{{Degree}}** — {{Institution}}, {{Year}}`
- Certifications: one `- ` bullet per item.
- ATS-friendly: standard headers only, no tables, no columns, no graphics, no emojis.
- Keep it concise (about one page). Vary wording from the base resume so it isn't copy-pasted.

Output ONLY the resume in markdown, starting with the `# {{Full Name}}` line."""
...
def _generate_cover_letter(job, base_resume, base_cover, candidate, llm_config,
                           mode, variant, analysis, template=None):
    """Generate a tailored cover letter with field-aware template context."""
    template = template or {}

    # Field-specific context for cover letter
    template_industry = template.get("industry_context", "")
    template_ats = ", ".join(template.get("ats_keywords", [])[:10])

    field_hint = ""
    if template_industry:
        field_hint = f"\nIndustry context: {template_industry}"
    if template_ats:
        field_hint += f"\nField-specific terms to use naturally: {template_ats}"
...
You adapt tone and terminology to the industry.
