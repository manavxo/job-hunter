...
# Scoring
# ---------------------------------------------------------------------------
# Role words that are too generic to signal relevance on their own.
GENERIC_TITLE_WORDS = {
    "manager", "coordinator", "officer", "lead", "specialist", "assistant",
    "associate", "senior", "junior", "sr", "jr", "i", "ii", "iii", "the", "of", "and",
}


def score_job(job, target_titles, min_salary=0):
    """Score a job 0-100 by relevance. Jobs with no title relevance are capped low."""
    score = 0
    title = job.get("title", "").lower()
    desc = job.get("description", "").lower()
    title_words = set(re.findall(r"[a-z]+", title))

    # Title match (0-45). A real match shares a meaningful (non-generic) word
    # like "project"/"program", not just "manager".
    best = 0
    for target in target_titles:
        t = target.lower().strip()
        if not t:
            continue
        if t in title:                       # full phrase, e.g. "project manager"
            best = max(best, 45)
            continue
        shared = set(re.findall(r"[a-z]+", t)) & title_words
        meaningful = shared - GENERIC_TITLE_WORDS
        if meaningful:                       # shares a domain word
            best = max(best, 30 if len(meaningful) >= 2 else 22)
        elif shared:                         # only generic overlap ("manager")
            best = max(best, 8)
    score += best
    ...
    # Relevance gate: a job with no meaningful title match isn't a real result —
    # keep it well below anything that actually matches a target title.
    if best < 20:
